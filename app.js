(() => {
  ...
  function dayHtml(d) {
    ...
    return `<div class="day-content"><div class="day-hero"><div><div class="date">DAY ${d.dayNo} · ${esc(d.city)} · ${esc(jpDate(d.date))}</div><h3>${esc(d.title)}</h3><p>${esc(d.lead)}</p></div><div class="day-number">${d.dayNo}</div></div>${route}<div class="timeline">${d.events.map(eventHtml).join('')}</div></div>`;
  }
  ...
  function renderToday() {
    ...
    $('#todayContent').innerHTML = dayHtml(d);
  }
  ...
  function renderFilters() {
    ...
    $$('.filter').forEach(b => b.onclick = () => { filter=b.dataset.filter; renderFilters(); renderSpots(); });
  }
